...
An empty chat opens. The agent waits for your message; no task is sent automatically at start-up. For example, this is the request that was sent in the recorded run, typed into the editor and sent with Enter or the send button. That button reads **Send ↵** outside the discussion, which is how the recorded run was made, and **Discuss ↵** while the discussion is on:
...
That is already a multi-file task: a parser, data processing, a command-line interface, a report and tests. You can open and run its result yourself, with one caveat about how it got there: the first turn ran out of the 300-second limit and is saved as an error, and the working result comes from the recorded continuation. The text does not select a pre-recorded scenario: the worker receives exactly your request.

### Who is working right now

A plain `./jev` starts in the guided mode, so the first thing that happens is a discussion rather than a route: up to three clarifying questions while the task is still vague, a review screen for your answers, and a plan you accept with **Start work**, which runs exactly the version you reviewed. A detailed request goes straight to the plan. Only then does **Jev** answer a Choice: whether this needs the file-editing mode, project research or a conversational answer. **Codex** reads the material, writes code, runs commands and formulates the answer. **Harness** runs the checks, saves the results and limits how far the loop may continue. After an attempt Jev proposes completion, a repair or a clarification, and separately judges through Noul how well the work matches the request.

The terminal shows the chain of stages:

```text
REQUEST → JEV ROUTE → POLICY → CODEX → CHECKS → JEV REVIEW → RESULT
                                                ↑                  │
                                                └───── repair ─────┘
```
...
Ask it next to add a filter by service, to keep the previous report format, and to check that filtering does not break the deduplication. The next turn receives the history and works in the same folder. You can return to it after closing the terminal with this command:
