...
**`--worker codex|claude`** — кто выполняет работу. Выбор сохраняется в сессии, поэтому `--resume` продолжает тем же исполнителем. Codex запускается в песочнице ядра ОС с выключенной сетью. У Claude Code такого механизма нет, поэтому гарантия «без сети и без установки пакетов» держится иначе: ему не выдаётся Bash, а `--restricted` игнорирует твой `~/.claude/settings.json` и запирает файловые инструменты в рабочую папку. Проверено запуском: попытка записать файл вне рабочей папки отклоняется. Проверки в обоих случаях запускает harness, а не исполнитель.

```bash
jevis --worker claude            # выполнять через Claude Code
jevis --worker codex             # через Codex (по умолчанию)
jevis --doctor                   # показывает, какие исполнители найдены
```

Claude Code обычно не лежит в PATH — он внутри десктопного приложения, и агент находит его сам. Путь можно задать через `CLAUDE_WORKER_BIN`. Авторизация отдельная от приложения: `claude auth login`, по подписке.

**`--demo`** — полный ход без Codex и без ключа TypeSafe. Заменены только два сетевых участка: судья отвечает по схеме заданных вопросов, а вместо Codex работает демо-исполнитель, который создаёт два небольших реальных файла. Маршрут, отбор контекста, выполнение проверок и правила завершения — рабочий путь, не запись. Каждый синтетический ответ помечен: модель `SYNTHETIC-DEMO-NOT-JEV`, нулевой usage и поле `synthetic` в событии, поэтому демо-прогон нельзя принять за измерение. Демо-исполнитель пишет файлы, поэтому `--demo` не сочетается с `--project`.

```bash
./jev --mode plan --jev-mode assist
./jev --jev-mode observe
./jev --jev-mode off
./jev --demo                        # посмотреть ход целиком без установки Codex
./jev --demo --mission cryptolab    # демо на контракте: остановка вместо ложной приёмки
```
...
./jev --replay last                 # пересчитать решения сессии, без моделей
./jev --prompt 'Изучи этот проект'  # заполнить редактор, не отправлять
```

`--replay` читает только `events.jsonl` и пересчитывает каждое записанное решение дважды: под порогами, которые действовали на момент хода, и под сегодняшними. Первое проверяет, что код решения не изменил поведение на реальной истории; второе показывает, что сделало бы изменение порога. Рабочая папка для этого не нужна, вызовов Jev и Codex нет, код возврата `2` означает расхождение. Пороги и версия политики лежат в `jev_agent/decision.py` и записываются в `result.json` каждого хода.
