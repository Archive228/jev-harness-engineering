...
Готовый чат со свободным вводом, реальными действиями Codex и решениями Jev: [запуск и первый запрос](./terminal-agent/README.md). Команда `terminal-agent/jev` открывает пустой чат. Напиши обычную идею: «Хочу что-нибудь полезное про крипту». Агент уточнит результат, предложит план с критериями проверки и начнёт исполнение после кнопки **«Начать работу»**. План можно поправить своими словами; точное задание для исполнителя видно в отдельной вкладке. Вопросы и ответы сохраняются между запусками.

**Enter** или кнопка «Обсудить» отправляет сообщение целиком; **Ctrl+J** добавляет строку. Shift+Enter работает, если его поддерживает терминал, а Ctrl+D остаётся дополнительным способом отправки. Вставка сохраняет все абзацы и ничего не отправляет автоматически. Для исполнения заранее точного задания без обсуждения есть `--direct` и `/direct`.

В версии 2: раскрываемые карточки инструментов, панель решений Jev по F6, поиск сессий, черновики, просмотр файлов и diff, экспорт Markdown, режимы plan и Jev assist/observe/off. `terminal-agent/jev --mission cryptolab` создаёт отдельный учебный проект учёта крипто-сделок с семью заранее заданными тестами. [Глава о готовом терминальном агенте](./terminal-agent/ARTICLE-TERMINAL-V2.md), [разбор исходников Pi/PiJev/OpenCode/Crush](./terminal-agent/research/upstream-audit.md), [проверки v2](./terminal-agent/verification/v2/README.md).

Обновлённый интерфейс использует розовый акцент, видимые кнопки навигации и отдельный итог ответа. После успешного завершения «Ход работы» сворачивается; его можно раскрыть кликом или Ctrl+O. Кнопки «Файлы», «Копировать» и «Экспорт» помогают забрать результат, а «Стоп» доступен во время работы. [Проверка обновлённого интерфейса](./terminal-agent/verification/ui-pink/README.md) отделена от исторического живого прогона v2.

Новый путь **идея → вопросы → план → исполнение** проверен живым созданием терминального криптокалькулятора. [Протокол, код, тесты и скриншоты](./terminal-agent/verification/guided/README.md). В продукте используются адаптации открытых исходников OpenCode, Hermes и Codex; [точные файлы, лицензии и границы переноса](./terminal-agent/research/ATTRIBUTION.md) сохранены в репозитории.
