...
Готовый чат со свободным вводом, реальными действиями Codex и решениями Jev: [запуск и первый запрос](./terminal-agent/README.md). Команда `terminal-agent/jev` открывает пустой чат. **Enter** или кнопка «Отправить» отправляет сообщение целиком; **Ctrl+J** добавляет строку. Shift+Enter работает, если его поддерживает терминал, а Ctrl+D остаётся дополнительным способом отправки. Вставка сохраняет все абзацы и не запускает задачу. История, файлы и протоколы сохраняются между сообщениями.
