...
> `confidence` is not a separate signal. It is a function of `probabilities`, and for `n` options the documentation reduces it to `(n × p_max − 1) / (n − 1)`.

We checked that formula against all 161 Choice and Score answers saved in the kit. The largest deviation is 0.015, which is rounding to two decimals.
...
The request carries no `seed` and no `temperature`. TypeSafe's own repeats give a standard deviation around 0.01 on the probability, and their Choice label moved on two questions out of eight.
